...
* 双向链表可以借助两个额外的数组来实现，分别用`l[i]`和`r[i]`表示编号为`i`的元素的左节点和右节点。 
...
* 二叉树的定义：
  + 一个标准的二叉树可以用「结构体 + 指针」的方式实现：
    ...
    struct Seq {
        ...
        Seq() : have_value(false), val(0), l(nullptr), r(nullptr) {}
    };
  ...
  + 用数组也可以实现二叉树。需要三个数组，分别用于记录每个元素的左右子节点的编号以及该元素存放的数值（结构体）。
    ...
    int l[max_node_num], r[max_node_num];
...
* 对于树而言，递归和基于DFS的操作常常紧密结合。深入理解递归是进阶的重要前提。
  关于树的全面解析，参见[树、二叉树及相关算法 · 数据结构与算法 01](https://hliangzhao.cn/articles/000001650599093c2b7e233229d444b985efe2a73727594000)。
* 图也有DFS和BFS。和树一样，前者可以通过递归来实现，后者可以借助队列来实现。
  关于图的全面解析，参见[图及相关算法 · 数据结构与算法 02](https://hliangzhao.cn/articles/0000016508710865a4a2f5537d34a5f95093ca825bdbae6000)。
* `p6-7`中给出了一种基于`qsort`和自定义compare算法实现「数组排序后保留原index」的一种方法。掌握之。
* `p6-10`是一道关于STL容器的综合使用范例。
### TODO

* 完成第4章剩下的8道习题。 
* 完成第5章剩下的6道习题。
* 将第6章的习题整理至hliangzhao.cn站点。
* 完成第6章剩下的2道习题。
